...
**多设备接收器**：一个 Unifying 接收器可以同时带多台设备。`--all` 会列出同一接收器上的全部设备：

```
mouse-tray.exe --all
```

日常轮询（`--once`）仍只读每个接收器的第一台设备。这是刻意的取舍：应用每轮都会**新起一个进程**，
槽位发现结果无法跨轮次保留，而探测空槽位要等到超时，继续扫完剩余槽位会让"只有一个鼠标"的
常见情形每次轮询多花约 0.7 秒，且应用本身只使用第一台设备的读数。实测 `--once` 耗时未变。
