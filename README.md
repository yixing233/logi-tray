...
| 版本 | 文件 | 内存占用（空闲） | 适合 |
| --- | --- | --- | --- |
| **完整版** | `logi-tray-v1.2.1.zip`（约 1.4 MB） | 工作集约 63 MB / 提交约 15 MB | 想要亚克力毛玻璃、三种图标样式与主题切换 |
| **轻量版** | `logi-tray-lite-v1.2.0.zip`（约 0.9 MB） | 工作集约 51 MB / 提交约 12 MB | 只要电量数字，希望占用尽可能低 |
| **多品牌版** | `multi-tray-v1.2.0.zip`（约 0.7 MB） | 工作集约 61 MB / 提交约 15 MB | 有键盘/鼠标/耳机等多台不同品牌设备 |
...
| 关于页与检查更新 | ✅ | ✅ | ✅ |
| 按来源单独开关 | ❌ | ❌ | ✅ |
| 休眠设备显示睡前电量 | 图表内前向继承 | 图表内前向继承 | ✅ 卡片小字 |
...
两版的**应用图标刻意不同**：完整版是罗技「G」+ 电池，多品牌版是深色圆徽章内的
**三根高低电量条**（象征多台设备各自的电量），配色用本应用自己的科技蓝而非罗技绿。
图标由 `tools/make_multi_icon.py` 生成，可复现。
...
设备休眠（或拔掉接收器）后大号数字始终是 `--%` —— 那是**当前**读数，读不到就是读不到。
但下面会多一行小字，说明它**睡前还剩多少**：
...
记录写在 `%LOCALAPPDATA%\multi-tray\battery-history.json`，按设备的稳定标识（`DeviceReading.Key`）索引，
因此换 USB 口、重启程序都还能认出来。三条设计约束：

- **只用于显示**。历史值单独存在 `LastKnownPercent`，**绝不写回 `Percent`** ——
  否则大号数字会用旧值冒充当前值，让人以为「现在还有 77%」，比不显示更糟。
  排序、托盘图标与低电量提醒也一律只看在线读数。
- **认不出就不猜**。设备休眠后标识可能退化（罗技读取器解析不到任何设备时，键会从
  `logitech:PRO X Wireless` 变成 `logitech:unknown`）。这时只在**同一品牌前缀下只有一个候选**
  时才回退采用；有多个候选宁可留空，也不张冠李戴。
- **会过期**。超过 30 天（`BatteryHistory.MaxAgeDays`）的记录不再显示，并在下次写入时清除；
  最多保留 64 台设备的历史。

#### 已支持与验证情况

协议来自公开实现（ATK 除外，见下）。经**实机连续读数**验证，结果如下：

| 品牌 | 实现依据 | 实机验证 |
| --- | --- | --- |
| 罗技 | 本仓库 native 读取器（HID++ 2.0，5 条特性回退） | ✅ 53 项单测 + 实机稳定读到 |
| 迈从 MCHOSE | [rafagfran/mchose-v9-pro-battery-tray](https://github.com/rafagfran/mchose-v9-pro-battery-tray)（同型号 VID/PID） | ✅ 电量实机 **30%，稳定**；✅ 充电状态经**双向采样校准**（放电 / 充电） |
| ATK Z87 键盘 | 官方网页驱动 hub.atk.pro v3.2.27 的 WebHID 抓包（非公开实现） | ✅ 实机 **100%，充电中**，可靠性 **20/20** |

罗技用 HID++ 2.0；迈从用 64 字节请求 `55 65 01`；ATK Z87 键盘用第三套协议。

**迈从耳机的充电状态经真机双向采样校准。** 它的响应第 4 字节（`[3]`）是状态位：

| `[3]` | 实测场景 | 含义 |
| --- | --- | --- |
| `0x00` | 插上充电器后连续 6 次采样（电量 30%） | **充电中** |
| `0x02` | 明确放电使用时连续 15+ 次采样 | **放电中** |
| 其它 | 未观测到（`0x01`、`0x03` 等） | 显示「充电状态未知」，不下结论 |

> 早先的版本把它按「1/2 → 充电中、3 → 已充满」解释，那是**照抄上游后凭字面猜的**，
> 从未校准过，而且**两个方向都错了**：真实的 `0x02` 是放电，`0x00` 才是充电。
> 结果是耳机一直在放电、界面却始终显示「充电中」，
> 并且这个虚假状态还会连带**压掉低电量提醒**。
>
> 上游参考实现（协议来源）至今对它"记录但不解释"，
> 理由是单一样本不足以断定含义 —— 本仓库采到双向样本后才做了映射。
> 尚未观测到「已充满」档；若要补全，请在满电时运行
> `multi-tray.exe --diag-mchose watch` 采样。

> **多品牌版没有历史图表**，但保留了「休眠设备显示睡前电量」这条最小信息：
> 设备离线时卡片会补一行小字（见上文「休眠设备的『上次电量』」）。

**ATK Z87 键盘**走的是官方驱动的私有指令，与同品牌鼠标协议**完全不同**
（旧实现误用了鼠标的 `GetBatteryLevel = 4`，因此一直读不到）。两个关键点：

1. **帧长必须是 64**。协议只用到 31 字节载荷，但该接口的输出报告是 64 字节，
   发 32 字节会被 `WriteFile` 拒绝（Win32 错误 87）。
2. **电量命令必须跟在 17 帧前导之后**（14 帧 LED 矩阵 + 2 帧键盘信息）。
   单发那一帧时设备把命令字节回成 `0xFF` —— 官方驱动源码里这是明确的「错误返回」：

```
前导: 01 00 1B 18 00 · 02 00 1B 18 18 · … 0E 00 1B 18 38 01
      0F 00 03 18 · 10 00 03 0E 18
电量: 11 00 1A 00 00 00 00 00 …                    ← 命令 0x1A
应答: 04 11 00 1A 00 00 00 00 64 02 …              ← [8]=0x64(100) [9]=0x02(充电中)
```

> 该键盘会暴露十余个 HID 接口，**只有一个能读电量**
> （`UP=0xFF1C IN=64 OUT=64`）。程序按「输出+输入报告长度」降序逐个试读，
> 试到在线就停 —— 早先「先去重再试读」会让它永远试不到正确的接口。
